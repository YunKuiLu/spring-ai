...
import static io.modelcontextprotocol.server.McpStatelessServerFeatures.AsyncPromptSpecification;
import static io.modelcontextprotocol.server.McpStatelessServerFeatures.AsyncResourceSpecification;
...
public class McpStatelessServerAutoConfigurationIT {
	...
	static class AsyncTestMcpSpecsComponent {
		...
		public Mono<McpSchema.GetPromptResult> greeting(
			...
			return Mono.just(new McpSchema.GetPromptResult("Greeting",
					List.of(new McpSchema.PromptMessage(McpSchema.Role.ASSISTANT, new McpSchema.TextContent(message)))));
		}
